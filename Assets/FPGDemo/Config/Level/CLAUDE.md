...
- `coverSlots` 是房间的独立掩体真源：每项保存 `FpgCoverEntityView` prefab、正耐久、掩体 pose、玩家到达 pose 和有效的 `FpgCoverCameraProfile`；每房至少一个且恰有一个初始掩体，玩家到达点的 X 不能重合。缺失或无效 Profile 必须 fail-closed，不回退到 `D0ThreeCProfile`。
