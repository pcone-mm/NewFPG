...
- `FPG_Luan_Attack_Summon.asset` 的召唤载荷槽配置为 `ReplaceOwner + OwnerPosition`，两个召唤数量字段为 `0`；同 Tick 后续的独立 `SelfDestructOwner` 节点通过 Event ID 绑定该 Summon。
- 240 tick 到达攻击 Ready 后开始 `die&broken` 演出；44 tick windup 只控制释放时机，结束前不调用 sink、不杀死 Luan。
...
- Hudie 通过 roster 与统一 Spawn Queue 成功入队并返回 `Queued` 后，才提交召唤成功事件；等待该结果的 `SelfDestructOwner` 节点再由正式 `ForceDeath -> EnemyDied -> EncounterRuntime.MarkEnemyDead` 链路结束 Luan。
- 正常 Retry 不杀死 Luan；ReplaceOwner 的固定容量或状态拒绝属于系统故障，不提交技能成功与 owner 死亡结果，战斗按统一契约 fail-closed。不得按 Luan/Hudie ID 绕过通用事务；固定池、队列、hitbox、递归和稳定 ID 容量由同一召唤图预检，其中 L1 Challenge 当前为 11 个计划实体、1 次替换、12 个 roster 生命周期槽、7 个按类型与最大同时需求预热的实体池槽，房间点位需求仍为 2。
