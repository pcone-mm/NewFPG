...
v1 只面向普通战斗房，提供创建、复制、分组、标签、五类标记可视化摆放、结构校验和 CombatLab 单房试玩。以下内容暂不属于本工具：
...
打开 Unity 菜单 `FPG Demo > Room Editor`。窗口负责浏览房间、筛选、编辑资料、选择标记、查看校验结果和发起单房试玩；SceneView 是实际摆点画布。
...
错误会阻止一键试玩；警告允许单房试玩，但必须由内容负责人判断是否符合当前房间意图。
...
| 警告 | 没有出口插槽 | 可继续单房试玩；跨房系统接入前必须补齐或明确该房不能离开 |
| 警告 | 没有可到达点 | 可继续单房试玩；表示当前房间尚无策划声明的可达候选点 |
...
点击校验结果应定位到对应资料或标记。不要通过临时改 Scenario ID 让错误消失后再忘记恢复。
...
5. 选择原 D0 Scenario 做组合校验，确认玩家与每个 SpawnSlot 的 ID 都能解析。
6. 如需同时安装默认 CombatLab 绑定，执行 `FPG Demo > Room Migration > Migrate CombatLab And Install Default Room`；再做一键试玩。
...
1. 在 Room Editor 中选择当前房间和一个 D0 Scenario/Encounter 组合。当前 CombatLab 的固定表现只接受 `BattleScenarioConfig.AuthoredScenario`；选择尚未安装对应表现的其他 D0 场景会被预检拒绝。
2. 点击“一键试玩”。工具先执行房间结构校验和 Spawn ID 组合校验。
3. 只存在警告时可以继续；存在错误时必须修复后重试。
4. 工具以临时覆盖加载 CombatLab 并进入 Play Mode。运行时由 `FpgRoomInstance` 实例化环境与可破坏物，并向玩家和敌人生成流程提供姿态；默认 CombatLab 的 EnvironmentBlocker 仍使用场景静态绑定。
5. 退出 Play Mode 后，工具恢复进入试玩前的场景，不保存临时覆盖。

试玩前必须保存 RoomDefinition、其主分组/标签、D0 遭遇和环境 Prefab，且不能处于 Prefab Mode；工具不会替作者自动保存这些资产。一次正常试玩不应产生 `CombatLab.unity`、房间定义或遭遇资产 diff。若退出后这些资产变脏，先不要提交，记录复现步骤并交给程序检查临时覆盖清理流程。
...
- 打开房间、刷新 Prefab、切换筛选和一键试玩不应产生内容 diff。
...
最小技术检查：Unity 编译和 Console 无相关错误；房间结构与房间/遭遇组合校验结果正确；保存重开后数据一致；打开编辑器和一键试玩不产生 CombatLab 场景 diff。
...
- [ ] 校验错误阻止试玩，缺出口/可到达点只显示警告。
- [ ] 一键试玩使用所选房间与遭遇，退出后恢复原场景且不污染 `CombatLab.unity`。
...
| H-04 | 单房试玩闭环 | 选择通过组合校验的房间和 D0 遭遇 | 一键进入 CombatLab，退出后检查场景与 Git diff | 进入点/出生点符合配置，退出后原场景恢复且无内容污染 | 操作人、日期、录屏、Git 状态 | 待主管试玩/确认 | 战斗手感不属于房间工具技术验收 |
...
- 出口连接、权重、深度、奖励和房间类型规则应由后续系统拥有，不塞入 v1 编辑器的临时备注或隐藏字段。
...
`FPG Demo > Room Editor` 的 `Formal Encounter Preview` Foldout 只在内存中保存 Profile、Override、Seed、Depth、难度和访问序号，并调用与运行时相同的 Request/Plan 生成入口。正式预览和正式试玩请求都必须通过内存覆盖传参，不得把 Encounter 引用或试玩参数写回 Room、Profile、Override、场景或其他资产；旧 D0“一键试玩”按钮语义保持不变。
...
`Run in Active Formal Host` 仅在 Play Mode 且场景内恰有一个已加载、非持久化 `FpgEncounterHost` 时启动；缺少或存在多个正式 Host、Host 绑定/容量失败、或预览与运行时 digest 不一致时立即 Fail-Closed，绝不调用旧控制器或回退 CombatLab。当前工程没有预装正式 Encounter 场景，策划试玩前须先打开并配置独立正式场景。
