...
本目录保存正式 FPG 的 Editor-only 入口；`LevelAuthoring/`、`SkillAuthoring/` 与 `CombatTuning/` 有各自职责和局部指南，进入子目录后以其规则为准。
...
- `FpgWindowsDevelopmentBuild` 只构建 `BattleTest + FpgProductionSceneList` 的 Windows Development/AllowDebugging 包；它可临时替换 global/active Build Settings，但必须在 `finally` 恢复。Release 入口不得包含 `BattleTest`。
...
- 验证以 Unity 编译/Console、`AssemblyBoundaryTests.cs`、`BuildSettingsTests.cs`、`FpgRoomArtSceneContractTests.cs`、`FpgCoverCameraAuthoringTests.cs`、`FpgShootingTuningSnapshotTests.cs` 和对应子目录指南为准。
