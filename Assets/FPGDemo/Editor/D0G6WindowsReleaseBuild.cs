...
namespace FPG.Demo.Editor
{
    ...
    /// </summary>
    public static class D0G6WindowsReleaseBuild
    {
        ...
        public const string DefaultOutputRelativePath =
            "Builds/FPGDemoD0/G6WindowsRelease/NewFPGD0.exe";

        [MenuItem("FPG Demo/D0 2.5D/Build G6 Windows x64 Release")]
        ...
        /// FPG.Demo.Editor.D0G6WindowsReleaseBuild.BuildWindows64ReleaseFromBatch
        /// </summary>
        ...
        internal static BuildReport BuildWindows64ReleaseAt(string locationPathName)
        {
            ...
            if (string.IsNullOrWhiteSpace(outputDirectory))
            {
                throw new InvalidOperationException(
                    "D0 G6 Windows build output must include a directory.");
            }
            ...
            if (report.summary.result != BuildResult.Succeeded)
            {
                throw new BuildFailedException(
                    "D0 G6 Windows Release build failed. "
        ...
        private static bool TryValidateBuildInputs(string locationPathName, out string error)
        {
            ...
            {
                error = "D0 G6 build requires a Windows executable output path.";
                return false;
            }
            ...
            if (!string.Equals(
                    ...
                    StringComparison.OrdinalIgnoreCase))
            {
                error = "D0 G6 Windows build output must end with .exe.";
                return false;
            }
            ...
            if (string.IsNullOrWhiteSpace(outputDirectory))
            {
                error = "D0 G6 Windows build output must include a directory.";
                return false;
            }
