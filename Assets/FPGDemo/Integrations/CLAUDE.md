...
- `Feel/` 在默认 `Assembly-CSharp` 中把 `FPG.Unity` 发布的中立 supplemental feedback 事件路由到 More Mountains Feel；项目代码对 `MoreMountains.*` 的直接引用只允许留在该目录，并先读局部指南。
- 不修改 `Assets/ThirdParty/` 插件源码来绕过生命周期问题；插件升级或 API 变化单独处理并检查所有 Art Scene 绑定。
- 验证以 Unity 编译/Console、体积绑定的 `FpgRoomArtSceneContractTests.cs`/`FpgRoomArtSceneLoaderPlayModeTests.cs`，以及 Feel 的 `FpgSupplementalFeedbackTests.cs`/`FpgFeelEnemyHitAssetTests.cs` 为准；若增加 asmdef 必须同时审查 scene/prefab script reference。
