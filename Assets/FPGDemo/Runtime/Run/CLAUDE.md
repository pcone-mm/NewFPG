...
- `FpgEncounterRuntimeMode.BattleTestSandbox` 是显式开发模式：以空计划启动、不会自动完成房间，并只允许通过已校验的 external spawn 入口补敌。GM spawn 可绕过遭遇 Profile 的并发上限，但不得绕过固定容量、ID、placement、catalog 或实体预热合同。
- `FpgBattleGmCommandParser` 只解析 `gm.god`、`gm.ai`、`gm.spawn` 的纯文本语法；Catalog、房间出生点和 host 状态校验属于 Unity 层，不得引入 UnityEngine 或资产查询。
...
- traversal 期间玩家不可被新攻击选中，迟到 impact 只作过期消费；敌方投射物命中 EnvironmentBlocker 时按 GeometryId 解析实际掩体并扣减其耐久，只有当前掩体被击破才强制玩家暴露。表现完成不得反向推进这些领域事实。
- BattleTest 的无敌与 AI 开关只改变当前沙盒端口：无敌仍消费 impact 但施加零伤害，关闭 AI 必须取消待处理攻击并恢复预算；销毁 GM runtime 时恢复默认值，不能污染生产 session。
