...
- `D0ThreeCProfile` 只保留角色控制、移动与掩体过渡时序等角色合同；静态镜头 Pose、FOV 和裁剪面归 `Config/Level/CameraProfiles`，不得在适配器中提供隐式兜底。
...
- 修改后检查对应 `TryValidate/TryBuildData`、Unity 编译/Console、`FpgSkillDefinitionTests.cs`、`FpgPlayerSkillAssetContractTests.cs`、`FpgFormalSkillPresentationV3AssetTests.cs`、`FpgFormalEnemyRootMotionAssetTests.cs` 与现存的精确 Formal EditMode 合同。
