...
- `FpgFormalPlayerComposer` 根据 Boot 传入的 `FpgPlayableCharacterSelection` 暂存并校验玩家实体，再绑定 port factory、tick driver、director 和 presentation bridge；场景资产本身不保存已配置的玩家运行时引用。
...
- FormalRoom 玩家组装必须先在 inactive staging root 完成并通过全部校验，再激活表现；失败或清理时先让 director 释放 session/runtime bundle，再清 bridge、driver、factory 和实例化实体。
...
- 改玩家 catalog 选择、运行时组装或场景 authoring 合同时，再看 `Assets/FPGDemo/Tests/EditMode/FormalFirstAuthoringContractTests.cs`。
