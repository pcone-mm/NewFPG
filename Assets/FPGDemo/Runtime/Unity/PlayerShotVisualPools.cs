...
namespace FPG.Demo.Unity
{
    ...
    /// <summary>
    /// Fixed-capacity pool for the D0 secondary's target-local burst. Unlike
    /// <see cref="PlayerShotAreaPool"/>, this view never writes a horizontal
    /// ground circle: its ring is constructed in the presentation camera's
    /// screen-facing plane around a frozen target point.
    /// </summary>
    public sealed class PlayerShotTargetBurstPool : IDisposable
    {
        private PlayerShotTargetBurstView[] views;
        private bool[] activeSlots;
        private bool prepared;
            ...
            out string error)
        {
            if (root == null || material == null || capacity <= 0)
            {
                error = "Player target-burst root, material and a positive capacity are required.";
                return false;
            }
            ...
            if (prepared)
            {
                if (Capacity < capacity)
                {
                    error = "Prepared player target-burst pool capacity is below the requested capacity.";
                    return false;
                }

                for (int index = 0; index < Capacity; index++)
                {
                    views[index].SetPresentationCamera(presentationCamera);
                    views[index].ApplySorting(sortingLayerName, sortingOrder);
                }
            ...
            views = new PlayerShotTargetBurstView[capacity];
            activeSlots = new bool[capacity];
            try
            {
                for (int index = 0; index < capacity; index++)
                {
                    GameObject viewObject = new GameObject($"PlayerShotTargetBurst_{index}");
                    viewObject.transform.SetParent(root, false);
                    PlayerShotTargetBurstView view = viewObject.AddComponent<PlayerShotTargetBurstView>();
                    if (!view.TryPrepare(
                            material,
                            presentationCamera,
                            sortingLayerName,
                    ...
                    {
                        DestroyObject(viewObject);
                        error = $"Unable to prepare player target burst {index}: {viewError}";
                        Dispose();
                        return false;
                    }
            ...
            {
                error = $"Unable to prewarm player target bursts: {exception.Message}";
                Dispose();
                return false;
            }
        ...
        public bool TrySpawn(Vector3 center, Color color, float duration, float radius)
        {
            if (!prepared || duration <= 0f || radius <= 0f)
            {
                SpawnRejectCount++;
                return false;
            }
            ...
            views[slot].Activate(center, color, duration, radius);
            activeSlots[slot] = true;
            return true;
        }
    ...
    /// <summary>
    /// A screen-facing short ring used at the frozen impact point of a
    /// committed secondary shot. This is deliberately an effect-local shape,
    /// never a gameplay area indicator.
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class PlayerShotTargetBurstView : MonoBehaviour
    {
        private const int SegmentCount = 20;

        private readonly Vector3[] points = new Vector3[SegmentCount + 1];
        private LineRenderer lineRenderer;
        ...
        public bool TryPrepare(Material material, Camera nextPresentationCamera, out string error)
        {
            return TryPrepare(
                material,
            ...
            out string error)
        {
            if (material == null)
            {
                error = "Player target burst requires a material.";
                return false;
            }

            if (prepared)
            {
                SetPresentationCamera(nextPresentationCamera);
                ApplySorting(sortingLayerName, sortingOrder);
                error = string.Empty;
                return true;
            }
        ...
        public void SetPresentationCamera(Camera nextPresentationCamera)
        {
        ...
        public void Activate(Vector3 nextCenter, Color color, float nextDuration, float nextRadius)
        {
            if (!prepared)
            {
                return;
            ...
            duration = Mathf.Max(0.01f, nextDuration);
            elapsed = 0f;
            radius = Mathf.Max(0.01f, nextRadius);
            lineRenderer.startWidth = 0.115f;
            lineRenderer.endWidth = 0.075f;
            lineRenderer.startColor = color;
            lineRenderer.endColor = color;
            WriteRing(radius * 0.22f);
            lineRenderer.enabled = true;
            gameObject.SetActive(true);
        }
        ...
        public bool Advance(float deltaTime)
        {
            ...
            lineRenderer.startColor = color;
            lineRenderer.endColor = color;
            WriteRing(Mathf.Lerp(radius * 0.22f, radius, progress));
            if (progress < 1f)
            {
                return true;
            }
