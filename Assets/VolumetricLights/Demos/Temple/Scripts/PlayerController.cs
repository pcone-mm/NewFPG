...
namespace VolumetricLightsDemo {
    ...
    public class PlayerController : MonoBehaviour {
        ...
        void Update() {
            // Get Input
            InpHor = InputProxy.GetAxis("Horizontal");
            InpVer = InputProxy.GetAxis("Vertical");
            ...
            // Get Jump
            if (InputProxy.GetButtonDown("Jump") && thisCharacterController.isGrounded == true) {
                // Make jump timer = to jump time
                jumpTimer = jumpTime;
            }

            // Rotate
            transform.Rotate(0, InputProxy.GetAxis("Mouse X") * mouseSpeed, 0);
            ...
            yRotate += -InputProxy.GetAxis("Mouse Y") * mouseSpeed;
            // Clamp rotation
