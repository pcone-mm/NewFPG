...
- `Assets/Art/`、`Assets/Materials/`、`Assets/Resources/`、`Assets/Rendering/`、`Assets/Settings/` 和 `Assets/ThirdParty/` 是保留的源资源、渲染配置或外部资产，不是正式运行入口；复用前必须先确认 `Assets/FPGDemo/` 中存在真实引用和所有权边界。
...
- 中等以上设计或实现任务先使用 `Docs/Workflow/UnknownsMethodology.md`；玩家可见行为和人工体验结论不得由 Agent 猜测。
