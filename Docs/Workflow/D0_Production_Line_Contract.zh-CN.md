...
不得为 D0 新建绕开该图谱的平行 JSON、场景常量或临时 Inspector 字段。玩家主射 / 副射 Socket 与瞬时表现字段见 [D0 Fei 主射 / 副射表现配置](D0_Fei_Attack_Presentation_Configuration.zh-CN.md)。
...
| Spine | idle、主射、换弹 play/ready、副射蓄力/释放、受击、胜败；事件名记录 | enter、idle、受击、groggy、death、fast、volley、heavy；事件名记录 | 动画名与 `D0ActorPresentationDefinition` 一致 |
| 锚点 | 派生角色 Prefab 的技能 Socket、角色视觉根、护盾根 | `EntityPrefab` 内的视觉根、gameplay 根、投射锚点与弱点锚点 | Socket 槽位、层级和局部空间稳定；Stage 不创建枪口或攻击锚点 |
| 命中 | 不以视觉网格替代碰撞 | `EntityPrefab` 内 Body 2001、Weakpoint 2002 保持独立 | 命中体跟随 gameplay 根，且不由 Stage、Encounter 或 VFX 创建 |
| 特效/音频槽 | 主射、副射、命中、护盾、受击 | 入场、快攻、弹幕、重型、Break、死亡 | 每个招式写明 VFX 槽和音频 Cue 槽 |
...
| 玩家角色表现/技能 | 拥有角色视觉局部姿态、技能 Socket，以及主射/副射的枪口、弹道、蓄力和爆发表现参数 | Stage 临时枪口或副射目标代理 |
...
4. 将通过验收的资产绑定到 `D0ActorPresentationDefinition`；玩家派生 Prefab 由安装器按角色资产生成 Socket provider，再交由角色定义引用。
...
- 探身/缩回的表现节奏；探身改变暴露度，不能驱动角色位移。
- 护盾淡入、淡出、颜色与不透明度表现。
- 主射与副射镜头反馈幅度及回正时间。

调参入口：在 Project 中打开 `Assets/FPGDemo/Config/D0Slice/Definitions/Fei/D0_Fei_3C.asset`，或使用菜单 `FPG Demo/D0 2.5D/Open Camera & 3C Configuration`。进入 CombatLab Play Mode 后，Inspector 的“运行时预览”区域会把相机、准星、护盾和射击反馈改动即时应用到当前运行；需要更新攻击查询距离或输入缓冲时点击“重启战斗并应用全部”。普通调参不需要执行安装器，也不要在 `D0_CombatLab_Stage.asset` 或场景 Camera 上保留第二份相机数值。完整字段与生效时机见 [D0 3C 运行时预览与调参流程](D0_ThreeC_Runtime_Preview_Configuration.zh-CN.md)。

运行时规则：准星和输入缓冲由 3C Profile 应用；攻击查询最大距离由 3C Profile 覆盖；探身／缩回过渡会约束护盾显示淡出／淡入的最短时长；镜头后移仅影响画面，攻击射线会抵消该表现偏移。玩家 gameplay 根由 Scenario 的 `playerSpawnPointId` 选择 Stage SpawnPoint，玩家视觉局部姿态由角色表现定义提供；镜头构图验收不会反向改写这两类配置。伤害、护盾规则和 Break 仍由确定性战斗域处理。
...
`FpgDemoD0StageInstaller` 只负责首次搭建或修复环境、SpawnPoint、场景基础设施、对象池和序列化引用；它不把当前 Scenario 的角色或敌人烘焙成场景摆位，也不是日常 3C 数值调参或切换敌人的前置步骤。脚本升级或场景结构损坏时，须由获授权的执行者运行“FPG Demo/D0 2.5D/Install or Update Combat Slice”，随后等待 Unity 编译并复核场景合同。正常运行由 `BattleSessionHost` 在初始化/重启时读取 authored 配置并由生成系统实例化/绑定实体；缺少必要绑定仍必须拒绝初始化。
...
4. 玩家可完成瞄准、主射、副射、探身、缩回和 Break；战斗可走向胜利、失败和 F5 重开。
...
- [ ] 玩家枪口 Socket 与主射/副射瞬时表现只从角色技能表现配置解析，场景中不存在临时枪口或目标代理。
